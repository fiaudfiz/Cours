...
C'est le concept fondamental pour comprendre la portabilité (ou non) d'un code. Le **CPU** LIEN a un vocabulaire limité et "câblé" dans ses circuits : c'est son Jeu d'Instructions (ISA).
Le C (Haut niveau) : a = b + c; -->C'est portable (on peut le compiler pour n'importe quelle machine).
...
Note : C'est le rôle du compilateur (gcc, clang etc..) de traduire le C portable en ISA spécifique (x86 pour le PC, ARM pour le téléphone). Si on copie un fichier .exe (binaire x86) sur un Raspberry Pi (ARM), ça ne marche pas : ils ne parlent pas la même langue.
...
En cas de Crash (Segfault) : Si un programme plante avec un Segmentation Fault, c'est souvent parce que le Program Counter a essayé de lire une instruction à une adresse interdite (ex: pointeur nul). Le debugger te donnera la valeur du PC/RIP au moment de la mort pour te dire exactement où le code a cassé.

### 3. Le Cycle de la Vie (Fetch - Decode - Execute - Store)
